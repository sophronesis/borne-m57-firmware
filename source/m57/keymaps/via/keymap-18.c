...
This system maps custom RGB modes into QMK’s rgb_matrix_mode.

Each mode represents a distinct visualization strategy:

UI      → state visualization (layers, structure)
DEBUG   → diagnostics / development feedback
GAMING  → reactive effects (keypress animations)

These modes are selectable via Vial.
*/
...
/*
Maps RGB mode index → logical firmware mode.
...
/*
Currently active RGB mode (derived every frame from QMK state).
*/
...
Tracks which operating system is active.

Used for:
✔ RGB indication (visual feedback)
✔ future OS-specific behavior

Persisted to EEPROM to survive reboot.
*/
...
/*
Single source of truth for current OS.
*/
static os_type_t current_os = OS_WINDOWS;

/*
EEPROM layout (user space):
+0 → OS
+1 → Theme
*/
...
/*
Loads OS state at startup.
Includes safety guard for invalid EEPROM values.
*/
...
/*
Writes OS state only if changed (wear-safe).
*/
...
Stores currently active RGB theme.

Themes define color palette:
- base (background)
- layer overlays
- reactive highlight color

Persisted independently from RGB mode.
*/
...
/*
Load theme index from EEPROM safely.
*/
...
/*
Returns active theme struct used by ALL render layers.

Central abstraction point:
→ Changing theme automatically affects entire pipeline.
*/
static const rgb_theme_t* get_active_theme(void) {
    ...
    return &theme_ui; // fallback safety
}
...
These extend QMK keycodes with firmware-specific actions.

Design:
✔ explicit state control (OS / theme)
✔ Vial-compatible
✔ complements encoder-based control

Note:
SAFE_RANGE prevents overlap with standard keycodes.
*/
...
/*
Defines LED region used to visualize OS selection.

This decouples:
hardware layout → logical meaning

Easy to relocate without changing logic.
*/
static const uint8_t os_indicator_zone[] = {0,1,2,3};

/*
Number of LEDs in OS indicator zone.
*/
...
Prevents repeated calls to keymap_key_to_keycode()
which are expensive in render loop.
...
/*
Stores LED indices for modifier keys only.
Used for fast modifier highlighting.
*/
...
lower = fresher / more CPU
higher = stale / less CPU
*/
#define KEYCODE_CACHE_REFRESH_INTERVAL 100


/*
Rebuilds LED → keycode mapping from current layer.
*/
...
/*
Builds list of modifier LEDs once at init.

Avoids checking flags every frame.
*/
...
/*
============================================================
REACTIVE EVENT BUFFER
============================================================

Stores recent keypresses and drives animations.

Design:
✔ fixed-size ring buffer
✔ time-decay driven
✔ supports clusters (multiple LEDs)

Used only in GAMING mode.
*/

#define MAX_REACTIVE 8

typedef struct {
    uint8_t leds[4];   // LEDs involved in event
    uint8_t count;     // number of LEDs used
    uint16_t timer;    // start time
} reactive_cluster_t;
...
Behavior:
- Only active when FN layer is active (layer 1)
- Cycles through OS states
- Persists change immediately

return false ⇒ stop default encoder behavior
*/
...
Responsibilities:
✔ handle custom keycodes (OS / theme)
✔ generate reactive events
*/
...
/*
Each render_* function contributes to final LED output.
...
Mode-dependent:
GAMING mode enables reactive layer.
*/
