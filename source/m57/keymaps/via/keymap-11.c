...
This system maintains a cached mapping:

    LED index → keycode (per active layer)
...
is relatively expensive if repeated per LED per frame.

The cache solves this by:

    ✔ storing results once
    ✔ reusing them during rendering
...
3. Periodically (time-based refresh):
...
- Cache MUST NOT be assumed static
- Always rebuild when layer changes
- Do NOT rebuild every frame unnecessarily

------------------------------------------------------------
PERFORMANCE IMPACT
------------------------------------------------------------

✔ Converts repeated lookups to O(1)
✔ Reduces CPU load per frame
✔ Improves RGB responsiveness
✔ Keeps system WFI-friendly
...
Prevents:
    ...
    ❌ repeated keymap lookups (expensive)
    ❌ inconsistent behavior with dynamic keymaps
    ❌ stale key interpretation
*/
